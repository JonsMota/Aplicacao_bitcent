...
export interface FieldYearMonthProps {
    data?: Date
    dataMudou?: (data: Date) => void
}

export default function FieldYearMonth(props: FieldYearMonthProps) {
    const hoje = new Date()
            ...
            <Popover withArrow>
                <Popover.Target>
                    <Button className={`
                        ...
                        w-full sm:w-44 px-3
                    `}>{DateFormat.mmyy.format(data)}</Button>
                </Popover.Target>
                <Popover.Dropdown>
